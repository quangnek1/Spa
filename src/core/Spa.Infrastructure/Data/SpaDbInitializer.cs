...
namespace Spa.Infrastructure.Data
{
	public class SpaDbInitializer
	{
		...
		{
			new ApplicationRole { Name = "Administrator", Description = "Quản trị viên toàn quyền" },
			new ApplicationRole { Name = "Manager", Description = "Quản lý cơ sở Spa" },
			new ApplicationRole { Name = "Customer", Description = "Khách hàng" }
		};
		...
		private async Task SeedSystemSettingsAsync()
		{
			...
			{
				new SystemSetting { Name = "MaxCustomerPerSlot", Type = "Booking", Value = "3", Status = true },
				new SystemSetting { Name = "AdvanceBookingDays", Type = "Booking", Value = "30", Status = true },
				new SystemSetting { Name = "SpaOpenTime", Type = "Time", Value = "09:00", Status = true },
				new SystemSetting { Name = "SpaCloseTime", Type = "Time", Value = "21:00", Status = true }
			};
		...
		private async Task SeedSpaServicesAsync()
		{
			if (!await _context.Categories.AnyAsync())
			{
				...
				{
					new ServicePackage { DurationMinutes = 60, Price = 300000 },
					new ServicePackage { DurationMinutes = 90, Price = 450000 }
				}
				};
