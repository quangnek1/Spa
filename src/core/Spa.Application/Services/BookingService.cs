...
namespace Spa.Application.Services
{
	public class BookingService : IBookingService
	{
		...
		public async Task<bool> CheckAvailabilityAsync(DateTime startTime, int durationMinutes)
		{
			...
			var maxCustomerSetting = await _unitOfWork.SystemSettings.GetFirstOrDefaultAsync(s => s.Name == "MaxCustomerPerSlot" && s.Status == true);
			int maxCapacity = maxCustomerSetting != null ? int.Parse(maxCustomerSetting.Value) : 3; // Mặc định là 3 nếu chưa cấu hình
		...
		public async Task<BookingResponseDto> CreateBookingAsync(CreateBookingRequestDto request)
		{
			...
			bool isAvailable = await CheckAvailabilityAsync(request.ScheduledStartTime, package.DurationMinutes);
			if (!isAvailable)
			{
				throw new Exception("Khung giờ này đã kín lịch. Vui lòng chọn giờ khác.");
			}
			...
			decimal finalPrice = package.Price - discountAmount;
